/*
 * SS_Lib - Signal-Slot Library for C (C89 compatible version)
 *
 * Full feature parity with ss_lib.c, adapted for C89 constraints:
 *   - No // comments
 *   - Declarations at top of blocks
 *   - No <stdint.h> / <stdbool.h>
 *   - No designated initializers
 *   - No mixing declarations and code
 *   - No variadic macros in user code
 *
 * Uses the same conditional compilation flags as ss_lib.c so
 * ss_config.h works identically for both versions.
 */
...
typedef struct {
    ...
    unsigned char signal_used[SS_MAX_SIGNALS];
    unsigned char slot_used[SS_MAX_SLOTS];
    size_t signal_count;
    size_t slot_count;
#else
    /* Dynamic allocation */
    ...
    char* ns;  /* namespace - avoid C++ reserved word issues */
 ...
 * null-terminates. Returns number of characters copied, or -1 if
 * truncated.
 */
...
static void free_slot(ss_slot_t* slot) {
    size_t index;
    index = (size_t)(slot - g_context->slots);
...
void ss_cleanup(void) {
    ...
    if (g_context->ns) SS_FREE(g_context->ns);
    SS_FREE(g_context);
    g_context = NULL;
}
...
ss_error_t ss_connect(const char* signal_name, ss_slot_func_t slot,
                      void* user_data) {
...
#if SS_USE_STATIC_MEMORY
        size_t mi;
        for (mi = 0; mi < SS_MAX_SIGNALS; mi++) {
            if (g_context->signal_used[mi]) {
                total_slots += g_context->signals[mi].slot_count;
            }
        }
#else
        ss_signal_t* ms = g_context->signals;
        while (ms) {
            total_slots += ms->slot_count;
            ms = ms->next;
        }
#endif
...
int ss_signal_exists(const char* signal_name) {
    ...
    exists = (find_signal(signal_name) != NULL);
...
ss_error_t ss_signal_unregister(const char* signal_name) {
...
#if SS_USE_STATIC_MEMORY
    {
        ...
        while (curr) {
            size_t si;
            for (si = 0; si < SS_MAX_SLOTS; si++) {
                if (&g_context->slots[si] == curr) {
                    g_context->slot_used[si] = 0;
                    g_context->slot_count--;
                    break;
                }
            }
...
ss_error_t ss_set_namespace(const char* namespace_name) {
    if (!g_context) return SS_ERR_NULL_PARAM;

    if (g_context->ns) {
        SS_FREE(g_context->ns);
        g_context->ns = NULL;
    }

    if (namespace_name) {
        g_context->ns = SS_STRDUP(namespace_name);
        if (!g_context->ns) return SS_ERR_MEMORY;
    }
